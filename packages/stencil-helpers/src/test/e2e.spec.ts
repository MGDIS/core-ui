...
import { darkBackground, renderAttributes, renderProperties } from './e2e.ts';

describe('test - e2e', () => {
  describe('darkBackground', () => {
    test.each([
      { condition: true, expected: '<span style="background:#999;display:inline-block;"><div></div></span>' },
      { condition: false, expected: '<div></div>' },
    ])('Should render html', ({ condition, expected }) => {
      const res = darkBackground(condition, '<div></div>');
