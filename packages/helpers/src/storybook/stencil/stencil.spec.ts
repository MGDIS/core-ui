...
import { filterArgs, stencilWrapper, getStoryHTML, getStorybookUrl, StorybookPreview } from '.';
...
describe('stencil', () => {
  ...
  describe('getStorybookUrl', () => {
    test.each([
      {
        params: {
          storybookBaseUrl: 'https://storybook.url',
          filePath: undefined,
        },
        expected: undefined,
      },
      {
        params: {
          storybookBaseUrl: 'https://storybook.url',
          filePath: '/atoms/mg-button',
        },
        expected: 'https://storybook.url--docs',
      },
    ])('Should render storybook url', ({ params, expected }) => {
      const result = getStorybookUrl(params.storybookBaseUrl, params.filePath);
