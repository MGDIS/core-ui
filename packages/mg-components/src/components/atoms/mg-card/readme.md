...
| ------------ | ------------- | ----------------------------- | --------------------------------- | ---------- |
| `radiusSize` | `radius-size` | Define the border radius size | `"medium" \| "small" \| "xsmall"` | `'medium'` |
...
| `--mg-c-card-overflow`                 | Defines the overflow property for the card. Unset by default, fallback on `--mg-c-card-overflow-default`. Left unset to preserve nested cards (e.g. `mg-panel > mg-card`); set it on the card to scroll or clip overflowing content (e.g. `auto`). |
