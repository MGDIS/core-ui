...
export class MgTabs {
  ...
  validateLabel(newValue: MgTabs['label']): void {
    if (!isValidString(newValue)) {
      throw new Error(`<mg-tabs> prop "label" is required and must be a string. Passed value: ${newValue}.`);
    }
  }
  ...
  validateSize(newValue: MgTabs['size']): void {
    if (!sizes.includes(newValue)) {
      throw new Error(`<mg-tabs> prop "size" must be one of: ${sizes.join(', ')}. Passed value: ${newValue}.`);
    }
  ...
  validateItems(newValue: MgTabs['items']): void {
    ...
    else throw new Error(`<mg-tabs> prop "items" is required and all items must be the same type: TabItem. Passed value: ${newValue}.`);
  }
  ...
  validateActiveTab(newValue: MgTabs['activeTab']): void {
    ...
    if (newValue === undefined) {
      ...
      throw new Error(
        `<mg-tabs> prop "activeTab" must be a number between ${this.startIndex} and ${this.tabs.length} and new value must be "activable". Passed value: ${newValue}.`,
      );
    }
  }
