...
const displayMessage = (page: Page, hasText: string, frame?: FrameLocator): Promise<void> =>
	(frame ?? page).locator('mg-button', { hasText }).click();

const expectedNotifications = (page: Page | FrameLocator, expected: number): Promise<void> =>
	expect(page.locator('#mg-notification-center mg-message')).toHaveCount(expected);
...
test('With one iframe', async ({ page }) => {
	...
	await testPage(
		page,
		'iframe.html',
		/Notification center with an iframe/,
		page.frameLocator('[title="index"]'),
		'index'
	);
});
...
test('With two iframes', async ({ page }) => {
	...
	await testPage(
		page,
		'inception.html',
		/Notification center with iframes/,
		page.frameLocator('[title="iframe"]'),
		'iframe'
	);
	...
	await testPage(
		...
		'inception.html',
		/Notification center with iframes/,
		page.frameLocator('[title="iframe"]').frameLocator('[title="index"]'),
		'index'
	);
});
